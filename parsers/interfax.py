...
from . import NewsLetter  # Убедитесь, что этот импорт работает корректно
        ...
        if category:
            ...
            if category == 'Экономика':
                ...
                date = datetime.datetime.strptime(date,
                                                  "%a, %d %b %Y %H:%M:%S %z")
                news.append(
                    NewsLetter(title, None, "ru", "Interfax", date, url,
...
if __name__ == "__main__":
    start_date = datetime.datetime.strptime("2024-05-30", "%Y-%m-%d").replace(
        tzinfo=pytz.timezone('Europe/Moscow'))
    end_date = datetime.datetime.strptime("2024-06-05", "%Y-%m-%d").replace(
        tzinfo=pytz.timezone('Europe/Moscow'))
