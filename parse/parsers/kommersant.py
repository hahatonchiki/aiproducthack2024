...
def get_newsletter_text(url):
    try:
        ...
        text = soup.find("div", class_="article_text_wrapper js-search-mark")
        if text:
            paragraph = text.find('p', class_='doc__text')
            if paragraph:
...
def get_from_feed(feed) -> List[NewsLetter]:
    ...
    for entry in feed.entries:
        ...
        if category:
            ...
            if category in ['Телекоммуникации', 'Бизнес', 'Hi-Tech']:
                title = entry.title
                url = entry.link
                date = entry.published
                # Изменение формата даты
                date = datetime.datetime.strptime(date, "%a, %d %b %Y %H:%M:%S %z")
                news.append(NewsLetter(title, None, "ru", "Коммерсантъ", date, url, datetime.datetime.now()))
    return news
